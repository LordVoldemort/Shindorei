...
World::AddSession_ (WorldSession* s)
{
    ...
    if (pLimit > 0 && Sessions >= pLimit && s->GetSecurity () == SEC_PLAYER && !HasRecentlyDisconnected(s))
    {
        ...
        sLog.outDetail ("File d'attente : Compte ( id = %u ) est dans la file d'attente en position (%u).", s->GetAccountId (), ++QueueSize);
        return;
    }
    ...
    if (pLimit > 0)
    {
        ...
        popu /= pLimit;
        popu *= 2;
        sLog.outDetail ("La population du serveur est (%f).", popu);
    }
}
...
void World::LoadConfigSettings(bool reload)
{
    ...
    SetMotd(sConfig.GetStringDefault("Motd", "Bienvenue sur votre serveur core ElvyCore"));
    ...
    {
        sLog.outError("Rate.Health (%f) doit être > 0. Utilisation de 1 à la place.",rate_values[RATE_HEALTH]);
        rate_values[RATE_HEALTH] = 1;
    }
    rate_values[RATE_POWER_MANA]  = sConfig.GetFloatDefault("Rate.Mana", 1);
    if (rate_values[RATE_POWER_MANA] < 0)
    {
        sLog.outError("Rate.Mana (%f) doit être > 0. Utilisation de 1 à la place.",rate_values[RATE_POWER_MANA]);
    ...
    {
        sLog.outError("Rate.Rage.Loss (%f) doit être > 0. Utilisation de 1 à la place.",rate_values[RATE_POWER_RAGE_LOSS]);
    ...
    {
        sLog.outError("Rate.RunicPower.Loss (%f) doit être > 0. Utilisation de 1 à la place.",rate_values[RATE_POWER_RUNICPOWER_LOSS]);
    ...
    {
        sLog.outError("Rate.Talent (%f) doit être > 0. Utilisation de 1 à la place.",rate_values[RATE_TALENT]);
    ...
    {
        sLog.outError("TargetPosRecalculateRange (%f) doit être >= %f. Utilisation de %f à la place.",rate_values[RATE_TARGET_POS_RECALCULATION_RANGE],CONTACT_DISTANCE,CONTACT_DISTANCE);
        rate_values[RATE_TARGET_POS_RECALCULATION_RANGE] = CONTACT_DISTANCE;
    }
    else if (rate_values[RATE_TARGET_POS_RECALCULATION_RANGE] > NOMINAL_MELEE_RANGE)
    {
        sLog.outError("TargetPosRecalculateRange (%f) doit être <= %f. Utilisation de %f à la place.",
    ...
    {
        sLog.outError("DurabilityLossChance.Damage (%f) doit être >=0. Utilisation de 0.0 à la place.",rate_values[RATE_DURABILITY_LOSS_DAMAGE]);
        rate_values[RATE_DURABILITY_LOSS_DAMAGE] = 0.0f;
    }
    rate_values[RATE_DURABILITY_LOSS_ABSORB] = sConfig.GetFloatDefault("DurabilityLossChance.Absorb",0.5f);
    if (rate_values[RATE_DURABILITY_LOSS_ABSORB] < 0.0f)
    {
        sLog.outError("DurabilityLossChance.Absorb (%f) doit être >=0. Utilisation de 0.0 à la place.",rate_values[RATE_DURABILITY_LOSS_ABSORB]);
        rate_values[RATE_DURABILITY_LOSS_ABSORB] = 0.0f;
    }
    rate_values[RATE_DURABILITY_LOSS_PARRY] = sConfig.GetFloatDefault("DurabilityLossChance.Parry",0.05f);
    if (rate_values[RATE_DURABILITY_LOSS_PARRY] < 0.0f)
    {
        sLog.outError("DurabilityLossChance.Parry (%f) doit être >=0. Utilisation de 0.0 à la place.",rate_values[RATE_DURABILITY_LOSS_PARRY]);
        rate_values[RATE_DURABILITY_LOSS_PARRY] = 0.0f;
    }
    rate_values[RATE_DURABILITY_LOSS_BLOCK] = sConfig.GetFloatDefault("DurabilityLossChance.Block",0.05f);
    if (rate_values[RATE_DURABILITY_LOSS_BLOCK] < 0.0f)
    {
        sLog.outError("DurabilityLossChance.Block (%f) doit être >=0. Utilisation de 0.0 à la place.",rate_values[RATE_DURABILITY_LOSS_BLOCK]);
    ...
    {
        sLog.outError("Le niveau decompression est de (%i) il doit être entre 1 et 9. Utilisation du niveau de compression par défaut (1).",m_configs[CONFIG_COMPRESSION]);
    ...
    {
        sLog.outError("GridCleanUpDelay (%i) doit être supérieur %u. Utilisation de cette valeur minimale.",m_configs[CONFIG_INTERVAL_GRIDCLEAN],MIN_GRID_DELAY);
    ...
    {
        sLog.outError("MapUpdateInterval (%i) doit être supérieur %u. Utilisation de cette valeur minimale.",m_configs[CONFIG_INTERVAL_MAPUPDATE],MIN_MAP_UPDATE_DELAY);
    ...
    if (reload)
    {
        ...
        if (val!=m_configs[CONFIG_PORT_WORLD])
            sLog.outError("WorldServerPort : L'option ne peut être change en rechargeant trinitycore.conf, utilisation de la valeur actuelle (%u).",m_configs[CONFIG_PORT_WORLD]);
    }
    else
    ...
    if (reload)
    {
        ...
        if (val!=m_configs[CONFIG_SOCKET_SELECTTIME])
            sLog.outError("SocketSelectTime : L'option ne peut être change en rechargeant trinitycore.conf, utilisation de la valeur actuelle (%u).",m_configs[CONFIG_SOCKET_SELECTTIME]);
    }
    else
    ...
    if (reload)
    {
        ...
        if (val!=m_configs[CONFIG_GAME_TYPE])
            sLog.outError("GameType : L'option ne peut être change en rechargeant trinitycore.conf, utilisation de la valeur actuelle (%u).",m_configs[CONFIG_GAME_TYPE]);
    }
    else
    ...
    if (reload)
    {
        ...
        if (val!=m_configs[CONFIG_REALM_ZONE])
            sLog.outError("RealmZone : L'option ne peut être change en rechargeant trinitycore.conf, utilisation de la valeur actuelle (%u).",m_configs[CONFIG_REALM_ZONE]);
    }
    else
    ...
    {
        sLog.outError("MinPlayerName (%i) doit être entre 1 et %u. Utilisation de la valeur 2.",m_configs[CONFIG_MIN_PLAYER_NAME],MAX_PLAYER_NAME);
    ...
    {
        sLog.outError("MinCharterName (%i) doit être entre 1 et %u. Utilisation de la valeur 2.",m_configs[CONFIG_MIN_CHARTER_NAME],MAX_CHARTER_NAME);
    ...
    {
        sLog.outError("MinPetName (%i) doit être entre 1 et %u. Utilisation de la valeur 2.",m_configs[CONFIG_MIN_PET_NAME],MAX_PET_NAME);
    ...
    {
        sLog.outError("CharactersPerRealm (%i) doit être entre 1 et 10. Utilisation de la valeur 10.",m_configs[CONFIG_CHARACTERS_PER_REALM]);
    ...
    {
        sLog.outError("CharactersPerAccount (%i) ne peut être inférieur à CharactersPerRealm (%i).",m_configs[CONFIG_CHARACTERS_PER_ACCOUNT],m_configs[CONFIG_CHARACTERS_PER_REALM]);
    ...
    {
        sLog.outError("HeroicCharactersPerRealm (%i) doit être entre 0 et 10. Utilisation de la valeur 1.",m_configs[CONFIG_HEROIC_CHARACTERS_PER_REALM]);
    ...
    {
        sLog.outError("SkipCinematics (%i) doit être entre 0 et 2. Utilisation de la valeur 0.",m_configs[CONFIG_SKIP_CINEMATICS]);
    ...
    if (reload)
    {
        ...
        if (val!=m_configs[CONFIG_MAX_PLAYER_LEVEL])
            sLog.outError("MaxPlayerLevel : L'option ne peut être change en rechargeant trinitycore.conf, utilisation de la valeur actuelle (%u).",m_configs[CONFIG_MAX_PLAYER_LEVEL]);
    }
    else
    ...
    {
        sLog.outError("MaxPlayerLevel (%i) doit être entre 1 et %u. Utilisation de la valeur %u.",m_configs[CONFIG_MAX_PLAYER_LEVEL],MAX_LEVEL,MAX_LEVEL);
    ...
    {
        sLog.outError("StartPlayerLevel (%i) doit être entre 1 et MaxPlayerLevel(%u). Utilisation de la valeur 1.",m_configs[CONFIG_START_PLAYER_LEVEL],m_configs[CONFIG_MAX_PLAYER_LEVEL]);
        m_configs[CONFIG_START_PLAYER_LEVEL] = 1;
    }
    else if (m_configs[CONFIG_START_PLAYER_LEVEL] > m_configs[CONFIG_MAX_PLAYER_LEVEL])
    {
        sLog.outError("StartPlayerLevel (%i) doit être entre 1 et MaxPlayerLevel(%u). Utilisation de la valeur %u.",m_configs[CONFIG_START_PLAYER_LEVEL],m_configs[CONFIG_MAX_PLAYER_LEVEL],m_configs[CONFIG_MAX_PLAYER_LEVEL]);
    ...
    {
        sLog.outError("StartHeroicPlayerLevel (%i) doit être entre 1 et MaxPlayerLevel(%u). Utilisation de la valeur 55.",
    ...
    {
        sLog.outError("StartHeroicPlayerLevel (%i) doit être entre 1 et MaxPlayerLevel(%u). Utilisation de la valeur %u.",
    ...
    {
        sLog.outError("StartPlayerMoney (%i) doit être entre 0 et %u. Utilisation de la valeur %u.",m_configs[CONFIG_START_PLAYER_MONEY],MAX_MONEY_AMOUNT,0);
        m_configs[CONFIG_START_PLAYER_MONEY] = 0;
    }
    else if (m_configs[CONFIG_START_PLAYER_MONEY] > MAX_MONEY_AMOUNT)
    {
        sLog.outError("StartPlayerMoney (%i) doit être entre 0 et %u. Utilisation de la valeur %u.",
    ...
    {
        sLog.outError("MaxHonorPoints (%i) ne peut être négatif. Utilisation de la valeur 0.",m_configs[CONFIG_MAX_HONOR_POINTS]);
    ...
    {
        sLog.outError("StartHonorPoints (%i) doit être entre 0 et MaxHonorPoints(%u). Utilisation de la valeur %u.",
    ...
    {
        sLog.outError("MaxArenaPoints (%i) ne peut être négatif. Utilisation de la valeur 0.",m_configs[CONFIG_MAX_ARENA_POINTS]);
    ...
    {
        sLog.outError("StartArenaPoints (%i) doit être entre 0 et MaxArenaPoints(%u). Utilisation de la valeur %u.",
    ...
    {
        sLog.outError("MinPetitionSigns (%i) doit être entre 0 et 9. Utilisation de la valeur 9.", m_configs[CONFIG_MIN_PETITION_SIGNS]);
    ...
    {
        sLog.outError("GM.StartLevel (%i) doit être entre StartPlayerLevel(%u) et %u. Utilisation de la valeur %u.",
    ...
    {
        sLog.outError("GM.StartLevel (%i) doit être entre 1 et %u. Utilisation de la valeur %u.", m_configs[CONFIG_START_GM_LEVEL], MAX_LEVEL, MAX_LEVEL);
    ...
    {
        sLog.outError("UpdateUptimeInterval (%i) doit être > 0, utilisation de la valeur par défaut : 10.",m_configs[CONFIG_UPTIME_UPDATE]);
    ...
    {
        sLog.outError("SkillGain.Crafting (%i) ne peut être négatif. Utilisation de la valeur 1.",m_configs[CONFIG_SKILL_GAIN_CRAFTING]);
    ...
    {
        sLog.outError("SkillGain.Defense (%i) ne peut être négatif. Utilisation de la valeur 1.",m_configs[CONFIG_SKILL_GAIN_DEFENSE]);
    ...
    {
        sLog.outError("SkillGain.Gathering (%i) ne peut être négatif. Utilisation de la valeur 1.",m_configs[CONFIG_SKILL_GAIN_GATHERING]);
    ...
    {
        sLog.outError("SkillGain.Weapon (%i) ne peut être négatif. Utilisation de la valeur 1.",m_configs[CONFIG_SKILL_GAIN_WEAPON]);
    ...
    {
        sLog.outError("MaxOverspeedPings (%i) doit être supérieur à 2 (ou 0 pour désactiver la vérification). Utilisation de la valeur 2.",m_configs[CONFIG_MAX_OVERSPEED_PINGS]);
    ...
    if (reload)
    {
        ...
        if (val!=m_configs[CONFIG_EXPANSION])
            sLog.outError("Expansion : L'option ne peut être change en rechargeant trinitycore.conf, utilisation de la valeur actuelle (%u).",m_configs[CONFIG_EXPANSION]);
    }
    else
    ...
    if (int clientCacheId = sConfig.GetIntDefault("ClientCacheVersion", 0))
    {
        ...
        if (clientCacheId > 0)
        {
            ...
            sLog.outString("Version du cache du client utilisé : %u", clientCacheId);
        }
        else
            sLog.outError("ClientCacheVersion ne peut être négatif %d, ignoré.", clientCacheId);
    }
    ...
    {
        sLog.outError("Visibility.Distance.Grey.Unit ne peut être supérieur à %f",MAX_VISIBILITY_DISTANCE);
    ...
    {
        sLog.outError("Visibility.Distance.Grey.Object ne peut être supérieur à %f",MAX_VISIBILITY_DISTANCE);
    ...
    {
        sLog.outError("Visibility.Distance.Continents ne peut être inférieur au max aggro radius %f", 45*sWorld.getRate(RATE_CREATURE_AGGRO));
        m_MaxVisibleDistanceOnContinents = 45*sWorld.getRate(RATE_CREATURE_AGGRO);
    }
    else if(m_MaxVisibleDistanceOnContinents + m_VisibleUnitGreyDistance > MAX_VISIBILITY_DISTANCE)
    {
        sLog.outError("Visibility.Distance.Continents ne peut être supérieur à %f",MAX_VISIBILITY_DISTANCE - m_VisibleUnitGreyDistance);
    ...
    {
        sLog.outError("Visibility.Distance.Instances ne peut être inférieur au max aggro radius %f",45*sWorld.getRate(RATE_CREATURE_AGGRO));
        m_MaxVisibleDistanceInInstances = 45*sWorld.getRate(RATE_CREATURE_AGGRO);
    }
    else if(m_MaxVisibleDistanceInInstances + m_VisibleUnitGreyDistance > MAX_VISIBILITY_DISTANCE)
    {
        sLog.outError("Visibility.Distance.Instances ne peutr être supérieur à %f",MAX_VISIBILITY_DISTANCE - m_VisibleUnitGreyDistance);
    ...
    {
        sLog.outError("Visibility.Distance.BGArenas ne peut être inférieur au max aggro radius %f",45*sWorld.getRate(RATE_CREATURE_AGGRO));
        m_MaxVisibleDistanceInBGArenas = 45*sWorld.getRate(RATE_CREATURE_AGGRO);
    }
    else if(m_MaxVisibleDistanceInBGArenas + m_VisibleUnitGreyDistance > MAX_VISIBILITY_DISTANCE)
    {
        sLog.outError("Visibility.Distance.BGArenas ne peut être supérieur %f",MAX_VISIBILITY_DISTANCE - m_VisibleUnitGreyDistance);
    ...
    {
        sLog.outError("Visibility.Distance.Object ne peut être inférieur au max aggro radius %f",float(INTERACTION_DISTANCE));
    ...
    {
        sLog.outError("Visibility.Distance.Object ne peut être supérieur à %f",MAX_VISIBILITY_DISTANCE-m_VisibleObjectGreyDistance);
    ...
    {
        sLog.outError("Visibility.Distance.InFlight ne peut être supérieur à %f",MAX_VISIBILITY_DISTANCE-m_VisibleObjectGreyDistance);
    ...
    if (reload)
    {
        if (dataPath!=m_dataPath)
            sLog.outError("DataDir : L'option ne peut être change en rechargeant trinitycore.conf, utilisation de la valeur actuelle (%s).",m_dataPath.c_str());
    }
    else
    {
        m_dataPath = dataPath;
        sLog.outString("Utilisation du dossier data : %s",m_dataPath.c_str());
    }
    ...
    sLog.outString("WORLD: Support VMap inclus. LineOfSight:%i, getHeight:%i",enableLOS, enableHeight);
    sLog.outString("WORLD: Dossier data des VMaps : %svmaps",m_dataPath.c_str());
    sLog.outString("WORLD: Les touches de configuration des VMaps sont : vmap.enableLOS, vmap.enableHeight, vmap.ignoreMapIds, vmap.ignoreSpellIds");
...
void World::SetInitialWorldSettings()
{
    ...
    {
        sLog.outError("Fichiers *.map corrects introuvables dans le dossier '%smaps' ou fichiers *.vamps/*.vmdir corrects introuvables dans le dossier '%svmaps'. Veuillez placer les fichiers *.map/*.vmap/*.vmdir dans les dossier appropriés ou corrigez la valeur DataDir dans trinitycore.Conf.",m_dataPath.c_str(),m_dataPath.c_str());
        exit(1);
    }

    ///- Loading strings. Getting no records means core load has to be canceled because no error message can be output.
    sLog.outString();
    sLog.outString("Chargement des Trinity strings...");
    ...
    ///- Load the DBC files
    sLog.outString("Initialisation des DBC...");
    LoadDBCStores(m_dataPath);
    DetectDBCLang();

    sLog.outString( "Chargement des Script Names...");
    objmgr.LoadScriptNames();

    sLog.outString( "Chargement des Instance Template..." );
    objmgr.LoadInstanceTemplate();

    sLog.outString( "Chargement des SkillLineAbilityMultiMap Data..." );
    spellmgr.LoadSkillLineAbilityMap();

    ///- Clean up and pack instances
    sLog.outString("Nettoyage des instances...");
    ...
    sLog.outString( "Chargement des tables locales_* ..." );
    ...
    sLog.outString(">>> >>> Tables locales_* chargées");
    sLog.outString();

    sLog.outString("Chargement de la table page_text...");
    objmgr.LoadPageTexts();

    sLog.outString("Chargement de la table gameobject_template...");   // must be after LoadPageTexts
    objmgr.LoadGameobjectInfo();

    sLog.outString("Chargement de la table spell_chain...");
    spellmgr.LoadSpellChains();

    sLog.outString("Loading Spell Required Data...");
    spellmgr.LoadSpellRequired();

    sLog.outString("Chargement de la table spell_elixir...");
    spellmgr.LoadSpellElixirs();

    sLog.outString("Loading Spell Learn Skills...");
    spellmgr.LoadSpellLearnSkills();                        // must be after LoadSpellChains

    sLog.outString("Chargement de la table learn_spell...");
    spellmgr.LoadSpellLearnSpells();

    sLog.outString("Chargement de la table spell_proc_event...");
    spellmgr.LoadSpellProcEvents();

    sLog.outString("Chargement de la table spell_bonus_data...");
    spellmgr.LoadSpellBonusess();

    sLog.outString("Chargement de la table spell_threat...");
    spellmgr.LoadSpellThreats();

    sLog.outString("Chargement de la table npc_gossip...");
    objmgr.LoadGossipText();

    sLog.outString("Loading Enchant Spells Proc datas...");
    spellmgr.LoadSpellEnchantProcData();

    sLog.outString("Chargement de la table spell_proc_item_enchant...");
    LoadRandomEnchantmentsTable();

    sLog.outString("Chargement de la table item_template...");                   // must be after LoadRandomEnchantmentsTable and LoadPageTexts
    objmgr.LoadItemPrototypes();

    sLog.outString("Chargement de la table page_text...");
    objmgr.LoadItemTexts();

    sLog.outString("Chargement de la table creature_model_info...");
    objmgr.LoadCreatureModelInfo();

    sLog.outString("Chargement de la table creature_equip_template...");
    objmgr.LoadEquipmentTemplates();

    sLog.outString("Chargement de la table creature_template...");
    objmgr.LoadCreatureTemplates();

    sLog.outString("Chargement de la table spell_script_target...");
    spellmgr.LoadSpellScriptTarget();                       // must be after LoadCreatureTemplates and LoadGameobjectInfo

    sLog.outString("Chargement de la table item_required target...");
    objmgr.LoadItemRequiredTarget();

    sLog.outString("Chargement de la table creature_onkill_reputation...");
    objmgr.LoadReputationOnKill();

    sLog.outString("Chargement de la table points_of_interest...");
    objmgr.LoadPointsOfInterest();

    sLog.outString("Loading Creature Base Stats...");
    objmgr.LoadCreatureClassLevelStats();

    sLog.outString("Chargement de la table creature...");
	...
	sLog.outString("Chargement de la table creature template addon data...");
    objmgr.LoadCreatureAddons();                            // must be after LoadCreatureTemplates() and LoadCreatures()

    sLog.outString("Chargement de la table creature_respawn...");   // must be after PackInstances()
    objmgr.LoadCreatureRespawnTimes();

    sLog.outString("Chargement de la table gameobject...");
    objmgr.LoadGameobjects();

    sLog.outString("Chargement de la table gameobject_respawn..."); // must be after PackInstances()
    objmgr.LoadGameobjectRespawnTimes();

    sLog.outString("Chargement des tables pool_*...");
    poolhandler.LoadFromDB();

    sLog.outString("Chargement des tables game_event_*...");
    gameeventmgr.LoadFromDB();

    sLog.outString("Chargement de la table game_weather...");
    ...
    sLog.outString("Chargement de la table npc_spellclick_spells...");
    objmgr.LoadNPCSpellClickSpells();

    sLog.outString("Chargement de la table spell_area...");          // must be after quest load
    spellmgr.LoadSpellAreas();

    sLog.outString("Chargement de la table areatrigger_teleport...");
    ...
    sLog.outString("Chargement de la table areatrigger_tavern...");
    objmgr.LoadTavernAreaTriggers();

    sLog.outString("Chargement de la table areatrigger_scripts...");
    objmgr.LoadAreaTriggerScripts();

    sLog.outString("Chargement de la table game_graveyard_zone...");
    objmgr.LoadGraveyardZones();

    sLog.outString("Chargement de la table spell_target_position...");
    spellmgr.LoadSpellTargetPositions();

    sLog.outString("Chargement de la table spell_pet_auras...");
    ...
    sLog.outString("Chargement de la table exploration_basexp...");
    objmgr.LoadExplorationBaseXP();

    sLog.outString("Chargement de la table pet_name_generation...");
    objmgr.LoadPetNames();

    sLog.outString("Loading the max pet number...");
    objmgr.LoadPetNumber();

    sLog.outString("Chargement de la table pet_levelstats...");
    objmgr.LoadPetLevelInfo();

    sLog.outString("Chargement de la table corpse...");
    objmgr.LoadCorpses();

    sLog.outString( "Chargement de la table mail_level_reward..." );
    ...
    sLog.outString("Chargement de la table skill_discovery_template...");
    LoadSkillDiscoveryTable();

    sLog.outString("Chargement de la table skill_extra_item_template...");
    LoadSkillExtraItemTable();

    sLog.outString("Chargement de la table skill_fishing_base_level...");
    objmgr.LoadFishingBaseSkillLevel();

    sLog.outString("Chargement des tables archievement_*...");
    achievementmgr.LoadAchievementReferenceList();
	sLog.outString("Chargement Achievement Criteria Lists...");
    achievementmgr.LoadAchievementCriteriaList();
	sLog.outString("Chargement Achievement Criteria Data...");
    achievementmgr.LoadAchievementCriteriaData();
	sLog.outString("Chargement Achievement Rewards...");
    achievementmgr.LoadRewards();
	sLog.outString("Chargement Achievement Reward Locales...");
    achievementmgr.LoadRewardLocales();
	sLog.outString("Chargement termine Achievements...");
    achievementmgr.LoadCompletedAchievements();

    ///- Load dynamic data tables from the database
    sLog.outString("Chargement Item Auctions...");
    auctionmgr.LoadAuctionItems();	
    sLog.outString("Chargement de la table auctionhouse...");
    auctionmgr.LoadAuctions();

    sLog.outString("Chargement des tables guild_*...");
    objmgr.LoadGuilds();

    sLog.outString("Chargement des tables arena_*...");
    objmgr.LoadArenaTeams();

    sLog.outString("Chargement des tables goup_* et groups...");
    objmgr.LoadGroups();

    sLog.outString("Chargement de la table reserved_name...");
    objmgr.LoadReservedPlayersNames();

    sLog.outString("Loading GameObjects for quests...");
    objmgr.LoadGameObjectForQuests();

    sLog.outString("Chargement de la table battlemaster_entry...");
    sBattleGroundMgr.LoadBattleMastersEntry();

    sLog.outString("Chargement de la table game_tele...");
    objmgr.LoadGameTele();

    sLog.outString("Chargement de la table npc_text...");
    objmgr.LoadNpcTextId();                                 // must be after load Creature and NpcText

    objmgr.LoadGossipScripts();                             // must be before gossip menu options

    sLog.outString("Chargement de la table gossip_menu...");
    objmgr.LoadGossipMenu();

    sLog.outString("Chargement de la table gossip_menu_option...");
    objmgr.LoadGossipMenuItems();

    sLog.outString("Chargement de la table npc_vendor...");
    objmgr.LoadVendors();                                   // must be after load CreatureTemplate and ItemTemplate

    sLog.outString("Chargement de la table npc_trainer...");
    objmgr.LoadTrainerSpell();                              // must be after load CreatureTemplate

    sLog.outString("Chargement de la table creature_movement...");
    WaypointMgr.Load();

    sLog.outString("Loading Creature Formations...");
    formation_mgr.LoadCreatureFormations();

    sLog.outString("Chargement de la table character_ticket...");
    objmgr.LoadGMTickets();

    ///- Handle outdated emails (delete/return)
    sLog.outString("Suppression des vieux courriers...");
    ...
    sLog.outString("Chargement des scripts...");
    sLog.outString();
    ...
    sLog.outString(">>> >>> Scripts chargés");
    sLog.outString();

    sLog.outString("Chargement de la table db_script_string...");    // must be after Load*Scripts calls
    objmgr.LoadDbScriptStrings();

    sLog.outString("Chargement de la table creature_ai_texts...");
    CreatureEAI_Mgr.LoadCreatureEventAI_Texts();

    sLog.outString("Chargement de la table creature_ai_summons...");
    CreatureEAI_Mgr.LoadCreatureEventAI_Summons();

    sLog.outString("Chargement de la table creature_ai_scripts...");
    CreatureEAI_Mgr.LoadCreatureEventAI_Scripts();

    sLog.outString("Initialisation des scripts...");
    ...
    sLog.outDebug("DEBUG:: Initialisation des temps de jeu et des timers");
    m_gameTime = time(NULL);
    m_startTime=m_gameTime;
    ...
    sLog.outString( "Lancement du système de map" );
    MapManager::Instance().Initialize();

    sLog.outString("Lancement du système d'events mondiaux..." );
    uint32 nextGameEvent = gameeventmgr.Initialize();
    m_timers[WUPDATE_EVENTS].SetInterval(nextGameEvent);    //depend on next event

    ///- Initialize Battlegrounds
    sLog.outString( "Lancement du système de Champ de Bataille" );
    ...
    sLog.outString("Chargement de la table transports...");
    MapManager::Instance().LoadTransports();

    sLog.outString("Loading Transports Events...");
    objmgr.LoadTransportEvents();

    sLog.outString("Suppressions des bannissements expirés...");
    loginDatabase.Execute("DELETE FROM ip_banned WHERE unbandate<=UNIX_TIMESTAMP() AND unbandate<>bandate");

    sLog.outString("Calcul de la prochaine mise a zéro des quêtes journalières...");
    ...
    sLog.outString( "WORLD: Royaume initialisé" );
	sLog.outString( "Vous utilisez l'émulateur ElvyCore, développé par Atlas" );
	sLog.outString( "Spécialement conçu pour Eternia et Elvyrion sous le nom d'E.TCore" );
	sLog.outString( "http://elvyrion.no-ip.org / http://eternia.serveftp.com" );
}
...
void World::DetectDBCLang()
{
    ...
    {
        sLog.outError("Incorrecte DBC.Locale! Doit être >= 0 et < %d (Utilisation de la valeur 0)",MAX_LOCALE);
