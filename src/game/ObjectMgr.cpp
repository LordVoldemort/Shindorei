...
void ObjectMgr::LoadItemPrototypes()
{
    ...
    for(std::set<uint32>::const_iterator itr = notFoundOutfit.begin(); itr != notFoundOutfit.end(); ++itr)
        sLog.outErrorDb("Item (Entry: %u) n'existe pas dans `item_template` mais referencer dans `CharStartOutfit.dnc`", *itr);	
}
    ...
    for (QuestMap::iterator iter = mQuestTemplates.begin(); iter != mQuestTemplates.end(); ++iter)
    {
        ...
        if (qinfo->QuestFlags & ~QUEST_TRINITY_FLAGS_DB_ALLOWED)
        {
            sLog.outErrorDb("Quest %u has `SpecialFlags` = %u > max allowed value. Correct `SpecialFlags` to value <= %u",
                qinfo->GetQuestId(),qinfo->QuestFlags  >> 24,QUEST_TRINITY_FLAGS_DB_ALLOWED >> 24);
    ...
    do
    {
        ...
        switch(tmp.command)
        {
            case SCRIPT_COMMAND_TALK:
            {
                ...
                // if(!objmgr.GetTrinityStringLocale(tmp.dataint)) will checked after db_script_string loading
                break;
            }
