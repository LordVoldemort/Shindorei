...
void Spell::SpellDamageSchoolDmg(uint32 effect_idx)
{
    ...
    if (unitTarget && unitTarget->isAlive())
    {
        switch (m_spellInfo->SpellFamilyName)
        {
                ...
                else if (m_spellInfo->TargetAuraState == AURA_STATE_CONFLAGRATE)
                {
                    ...
                    if (aura)
                    {
                        ...
                        if (aura->GetSpellProto()->SpellFamilyFlags[0] & 0x4) // Immolate
                            damage += pdamage * 3; // 9 secs
                        else if (aura->GetSpellProto()->SpellFamilyFlags[2] & 0x00000002) // Shadowflame
                            damage += pdamage * 4; // 8 secs
            ...
            {
                // Hammer of Wrath
                if (m_spellInfo->SpellFamilyFlags[0] & 0x00000080)
                {
                    float ap = m_caster->GetTotalAttackPowerValue(BASE_ATTACK);
