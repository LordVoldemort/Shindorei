...
bool ItemUse_item_levelplayer15(Player* pPlayer, Item* pItem, const SpellCastTargets &pTargets)
{
...
if (pPlayer->IsMounted()) // Is player mounted
{
    ...
    if(((pPlayer->getLevel()) + 15) > 80)
    {
        outstring_log("playerlevel+15 > 80 (%i+15=%i)",pPlayer->getLevel(),pPlayer->getLevel()+15);
    ...
    {
        outstring_log("Ajoute 15 levels");
        pPlayer->GiveLevel(pPlayer->getLevel()+15);
    }
    pPlayer->DestroyItemCount(pItem->GetEntry(),1,true);
    return true;
}
void AddSC_item_levelplayer15()
{
    Script *newscript;

      newscript = new Script;
    newscript->Name="item_levelplayer15";
    newscript->pItemUse = &ItemUse_item_levelplayer15;
    newscript->RegisterSelf();
    outstring_log("SD2: item_levelplayer 15 loaded");
}