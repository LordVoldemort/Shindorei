...
bool ItemUse_item_levelplayer10(Player* pPlayer, Item* pItem, const SpellCastTargets &pTargets)
{
...
if (pPlayer->IsMounted()) // Is player mounted
{
    ...
    if(((pPlayer->getLevel()) + 10) > 80)
    {
        outstring_log("playerlevel+10 > 80 (%i+10=%i)",pPlayer->getLevel(),pPlayer->getLevel()+10);
    ...
    {
        outstring_log("Ajoute 10 levels");
        pPlayer->GiveLevel(pPlayer->getLevel()+10);
    }
    pPlayer->DestroyItemCount(pItem->GetEntry(),1,true);
    return true;
}
void AddSC_item_levelplayer10()
{
    Script *newscript;

      newscript = new Script;
    newscript->Name="item_levelplayer10";
    newscript->pItemUse = &ItemUse_item_levelplayer10;
    newscript->RegisterSelf();
    outstring_log("SD2: item_levelplayer 10 loaded");
}