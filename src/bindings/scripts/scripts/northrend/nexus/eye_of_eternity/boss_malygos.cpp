...
struct TRINITY_DLL_DECL boss_malygosAI : public ScriptedAI
{
    ...
    void Reset()
    {
        ...
        uiEnrageTimer = 615000; //10 min

        if (pInstance)
            pInstance->SetData(DATA_MALYGOS_EVENT, NOT_STARTED);
    }
    ...
    void EnterCombat(Unit* who)
    {
        ...
        if (pInstance)
            pInstance->SetData(DATA_MALYGOS_EVENT, IN_PROGRESS);			
    }
    ...
    void JustDied(Unit* killer)
    {
        ...
        if (pInstance)
            pInstance->SetData(DATA_MALYGOS_EVENT, DONE);	
    }
